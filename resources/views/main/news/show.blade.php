...
@section('meta')
    ...
    <meta name="description" content="{{ $article->short_body }}">
    <meta name="title" content="Xenin - Darkrp | Dark Roleplay | US darkrp | dark rp server">
    <meta property="og:site_name" content="{{ env('APP_NAME') }}">
    <meta property="og:title" content="{{ $article->title }}">
    <meta property="og:description" content="{{ $article->short_body }}">
    ...
    <meta name="twitter:description" content="{{ $article->short_body }}">
        ...
        <div class="text-center mx-auto">
            ...
            <div class="card bg-brand-black shadow-sm" style="min-width: 250px !important;">
                <div class="card-body d-flex flex-row justify-content-center align-items-center">
                    <hovercard-component id="template_article_{{ $article->id }}" v-tippy-html slug="{{ $article->user->slug }}" avatar="{{ Storage::url($article->user->avatar) }}" rank="{{ $article->user->getRoleNames()->first() }}"></hovercard-component>
                    <a href="{{ route('users.show', $article->user) }}" class="avatar" v-tippy="{ html:  '#template_article_{{ $article->id }}', reactive : true,
